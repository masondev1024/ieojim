...
- **GitHub 서버 보호를 적용했습니다.** 2026-09-17 사용자 승인으로 저장소를 공개 전환한 뒤, `main`에 아래 PR·CI·관리자 보호 조건을 설정하고 API 재조회로 확인했습니다.
...
기존 비공개 상태에서는 요금제 제한으로 보호 요청이 HTTP 403으로 거절됐습니다. 사용자가 공개 전환을 승인해 현재는 무료 계정의 공개 저장소 보호 기능을 사용합니다. 추후 비공개로 되돌리려면 요금제와 보호 적용 여부를 함께 확인해야 합니다. [GitHub의 지원 범위](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-protected-branches/about-protected-branches)

승인된 설정을 다시 적용할 때도 API로 재조회해 실제 적용 여부를 확인합니다. 입력에는 `checks`만 사용합니다. `contexts`까지 함께 보내면 API가 HTTP 422로 거절합니다.
